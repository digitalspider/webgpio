...
public class WebGpioController implements InitializingBean {
	...
	public String listFiles(Map<String,Object> model) {
		...
		try {
			...
			createPinModel(request,response,model,pin,pinRequest,valueParam,pinValue,edge,dir,pinCycle,pinDuty);
		} catch (Exception e) {
	...
	private void createPinModel(HttpServletRequest request, HttpServletResponse response, int pin, PinRequest pinRequest, String valueParam, int pinValue, PinEdge edge, PinDirection dir, int pinCycle, int pinDuty) throws IOException {
		model.put("pin",pin);
